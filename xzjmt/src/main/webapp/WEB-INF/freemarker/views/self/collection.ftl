...
<section>
  <div class="row">
	...
	<#list pageCtx.itemList as collection>
		<#assign item=((itemMap[collection.itemId?string])!)>
		<#if collection_index==0||(collection_index)%3==0>
			<div class="row-fluid">			
		</#if>	
			<div class="span4"><a href="${wwwroot}/item/${(collection.itemId)!}"><img src="${staticroot}/${(item.picCover)!}"></a></div>
		<#if !collection_has_next||(collection_index+1)%3==0>			
			</div>
			<div class="blank"></div>
		</#if>			
	</#list>		
		
		<@p.pagination action=wwwroot+"/self/collection" pageSize=pageCtx.pageBean.pageSize />	
    </div>
